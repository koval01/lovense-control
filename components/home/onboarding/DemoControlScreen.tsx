    ...
    <OnboardingLayout title={t('onboardingDemoTitle')} subtitle={t('onboardingDemoSubtitle')} onSkipAll={onSkipAll}>
      ...
      <div ref={demoContainerRef} className="h-[62dvh] min-h-[560px] max-h-[760px] rounded-[var(--app-radius-card)] overflow-hidden border border-[var(--app-border)] bg-[var(--app-bg-elevated)] relative z-10 shadow-[var(--app-shadow)]">
        <ToyControlContainer
          ...
          onCommand={() => {}}
          interactive={false}
          demoAutoplay={activeStep?.panelView === 'float' || isWalkthroughComplete}
          demoPanelView={activeStep?.panelView}
        />
      </div>
      ...
      <button
        ...
        className={`mt-6 w-full h-11 rounded-xl transition-colors text-sm font-medium relative z-10 ${isWalkthroughComplete ? 'app-button-primary text-white' : 'bg-[var(--vkui--color_separator_secondary)] text-[var(--app-text-secondary)] cursor-not-allowed opacity-70'}`}
      >
