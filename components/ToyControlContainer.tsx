...
export function ToyControlContainer({
  ...
  useEffect(() => {
    ...
    if (panelView === 'float') {
      ...
      const pickNextTarget = (
        ...
        const distance =
          (usesSmallStep ? 0.14 + Math.random() * 0.18 : 0.32 + Math.random() * 0.35) *
          rangeY;
        ...
        const lateral =
          (Math.random() - 0.5) * Math.max(8, (maxX - minX) * 0.9);
      ...
      const animate = (now: number) => {
        ...
        if (holdUntilTs !== 0 && now >= holdUntilTs) {
          ...
          fromX = currentX;
          fromY = currentY;
          const { nextX, nextY } = pickNextTarget(
            fromX,
            fromY,
            minX,
            maxDriftX,
        ...
        if (holdUntilTs === 0) {
          const progress = Math.max(
            0,
            Math.min(1, (now - segmentStartTs) / segmentDurationMs)
          );
        ...
        const level = Math.round(
          Math.max(0, Math.min(100, (1 - currentY / rect.height) * 100))
        );
  ...
  const rootHeightClass = interactive
    ? 'h-[68dvh] min-h-[420px] md:h-[78dvh]'
    : 'h-full';
