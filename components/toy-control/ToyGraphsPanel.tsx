...
export function ToyGraphsPanel({
  ...
  return (
    <div className={`shrink-0 ${splitUltraCompact ? 'grid grid-cols-2 border-b border-[var(--vkui--color_separator_secondary)]' : 'flex flex-col'}`}>
      <AnimatePresence initial={false}>
        {toyList.map((toy) => (
          <motion.div
            ...
            className={`${splitUltraCompact ? 'min-w-0 border-r border-[var(--vkui--color_separator_secondary)] last:border-r-0' : '[&:not(:last-child)]:border-b [&:not(:last-child)]:border-[var(--vkui--color_separator_secondary)]'}`}
          >
            <MotorGraph
              toy={toy}
